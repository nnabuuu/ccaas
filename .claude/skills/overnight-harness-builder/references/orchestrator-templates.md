...
### 0. Nested Session Guard (CRITICAL)

When a harness is launched from inside a Claude Code session (common during development), `claude -p` will refuse to run due to the `CLAUDECODE` environment variable. The orchestrator MUST clear these variables before every `claude -p` invocation.

**Pattern**: Define a wrapper function and use it for ALL `claude -p` calls:

```bash
# At the top of harness.sh, define a safe claude wrapper:
claude_safe() {
  env -u CLAUDECODE \
...
Then replace all `claude -p` calls with `claude_safe -p`:

```bash
# WRONG — will fail inside Claude Code session:
claude -p "$prompt" --allowedTools "$TOOLS"

# RIGHT — always works:
claude_safe -p "$prompt" --allowedTools "$TOOLS"
```

**Why `env -u` instead of `unset`**: `unset` at the top of the script modifies the shell's own environment, which SHOULD propagate to child processes. However, in practice this is unreliable when the script is launched from certain sandbox environments. `env -u` creates a clean subprocess and is guaranteed to work.

**DO NOT use `unset` alone** — it has been observed to silently fail in some execution contexts. Always use `env -u` via the `claude_safe` wrapper.
...
Save changelog to: ${CHANGELOG_DIR}/v${version}-changelog.md"
  ...
  claude_safe -p "$prompt" --allowedTools "$GENERATOR_TOOLS"
}
```
...
score=$(grep -E '(总分|Total)[：:][[:space:]]*[0-9]+' "$EVAL_DIR/v${i}-eval.md" | head -1 | sed 's/.*[：:][[:space:]]*//' | sed 's|/.*||' | tr -d '[:space:]')
...
extract_score() {
  local eval_file="$1"
  # POSIX-compatible (works on macOS BSD grep + GNU grep)
...
Write your findings to evidence/ directory."
  ...
  claude_safe -p "$prompt" --allowedTools "$INVESTIGATOR_TOOLS" 2>&1 || {
...
for i in $(seq 1 $MAX_ROUNDS); do
  ...
  if [[ -n "$latest_evidence" ]]; then
    ...
    status=$(grep -E '判断[：:]' "$latest_evidence" 2>/dev/null | head -1 | sed 's/.*[：:][[:space:]]*//' | awk '{print $1}' || echo "UNKNOWN")
