...
# ============================================================
# live-lesson-observation-e2e Harness — Code Mode Iteration Loop
#
# Implements observation system: event pipeline, state derivation,
# real-time push, teacher observation panel.
# Artifact: backend/src/classroom/ + frontend teacher + manifest
...
# --- Configuration ---
TASK_NAME="live-lesson-observation-e2e"
MAX_ITERATIONS=8
SCORE_THRESHOLD=80
MIN_IMPROVEMENT=3
...
# Artifact files that the generator may modify
ARTIFACT_FILES=(
...
state_print_summary() {
  ...
  for it in $iters; do
    ...
    local it_score
    it_score=$(jq -r ".iterations[\"$it\"].score // \"—\"" "$STATE_FILE")
    echo "│  Iteration $it: $it_status (score: $it_score)"
    local steps
...
run_validation() {
  echo "  [Validation] Running backend build + jest observation + frontend build..."
  local errors=0
  ...
  echo "  [Validation] Building backend (nest build)..."
  if ! npx nest build 2>&1 | tail -10; then
    echo "  [Validation] nest build FAILED"
    errors=$((errors + 1))
  fi

  # --- Jest observation tests (GATE — unique to this harness) ---
  echo "  [Validation] Running jest observation tests..."
  if ! npx jest observation --no-coverage 2>&1 | tail -15; then
    echo "  [Validation] jest observation FAILED"
    errors=$((errors + 1))
  fi
  ...
  echo "  [Validation] Frontend typecheck..."
  if ! npx tsc --noEmit 2>&1 | tail -10; then
    echo "  [Validation] tsc FAILED"
    errors=$((errors + 1))
  fi

  echo "  [Validation] Frontend vite build..."
...
start_lesson_backend() {
  ...
  # Re-seed DB with updated manifest (generator may have modified it)
...
This is **iteration v1** (first round). You need to implement ALL 5 phases:
1. Event pipeline: addSystemEvent await, submit→observeTurn, step_complete, enriched context
2. State derivation: named constants, K-anchor awareness, mixed signal handling
3. Real-time push: broadcast observation after observeTurn, SSE observation field
4. Teacher panel: Glance dots, anchor progress, misconception sort, alert badges
5. Tests: ≥5 new tests in observation.service.spec.ts

**Priority**: Backend pipeline first (Phase 1+2) → Real-time push (Phase 3) → Frontend panel (Phase 4) → Tests (Phase 5).

**Key points**:
- Read existing observation.service.ts thoroughly — it already has 475 lines of logic
- Read existing tests — 33 tests must continue to pass
- Do NOT modify frozen files
- Run builds + jest to verify after changes
...
**Read eval report**: harness-workspace/live-lesson-observation-e2e/eval-reports/v${prev}-eval.md — tells you what lost points and how to fix it.
...
echo "║  Artifact: observation service + classroom + teacher panel  ║"
echo "║  Frozen: entities, DTOs, lesson, pages, student, pkgs      ║"
...
for i in $(seq $START_VERSION $MAX_ITERATIONS); do
  ...
  # ─── Step 3: Validation (backend build + jest observation + frontend build) ───
  echo ""
  echo "  [Step 3/6] Running validation (build + jest observation + frontend)..."
  if ! run_step "$i" "validation" "run_validation"; then
    echo "  Validation FAILED. Reverting artifact files and skipping to next iteration."
    cd "$REPO_ROOT"
    for f in "${ARTIFACT_FILES[@]}"; do
      git checkout -- "$f" 2>/dev/null || true
    done
    append_progress $i "FAIL" "Validation failed" "build or test error"
    continue
  fi
  ...
  git commit -m "feat(live-lesson): observation-e2e v${i} iteration" --allow-empty 2>/dev/null || true
  ...
  git commit -m "feat(live-lesson): observation-e2e v${i} eval — score ${score}/100" --allow-empty 2>/dev/null || true
  ...
  if [[ $prev_score -gt 0 && $score -lt $((prev_score - ROLLBACK_THRESHOLD)) ]]; then
    ...
    cd "$REPO_ROOT"
    git checkout HEAD~2 -- "${ARTIFACT_FILES[@]}" 2>/dev/null || true
    git add "${ARTIFACT_FILES[@]}" && \
    git commit -m "feat(live-lesson): observation-e2e v${i} REVERTED — regression ${prev_score}->${score}" 2>/dev/null || true
    continue
  fi
...
echo "  Artifact:  observation.service + classroom.service + TeacherShell"
echo "  History:   git log --grep='observation-e2e' --oneline"
echo ""
echo "  Next steps:"
