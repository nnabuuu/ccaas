...
# ============================================================
# recipe-book-multi-select Harness — Code Mode Iteration Loop
#
# Adds multi-select (Space to Add & Stay) to RecipePicker.
...
# --- Configuration ---
TASK_NAME="recipe-book-multi-select"
MAX_ITERATIONS=5
SCORE_THRESHOLD=100
MIN_IMPROVEMENT=1
...
This is **v1** (first iteration). RecipePicker already exists with single-select. You need to ADD multi-select on top.

**Priority**:
1. Add staging state + Space key handler
2. Add commit logic (Enter/Esc commit staging)
3. Add staging area UI (pills within picker)
4. Update list items with .staged class + green badge
5. Add CSS (staging area styles + flash animation)
6. Update placeholder text
7. Verify tsc + build

**Key**: Space toggles staging. Enter commits all. Esc commits all (never discard). Focus stays on search input.

**Changelog save to**: ${CHANGELOG_DIR}/v1-changelog.md"
  else
    prompt="${prompt}
...
This is **v${version}** iteration.

**Your starting point**: Code was modified by previous iterations. Read the current code first, then improve.

**Read eval report**: harness-workspace/recipe-book-multi-select/eval-reports/v${prev}-eval.md — tells you what failed and how to fix it.

**Strategy**: Fix Top Priority items from eval report. Don't redo dimensions that scored full marks.

**Changelog save to**: ${CHANGELOG_DIR}/v${version}-changelog.md"
  fi
...
# Progress — recipe-book-multi-select
...
echo "║  Feature: Multi-select with Space (Add & Stay)              ║"
...
for i in $(seq $START_VERSION $MAX_ITERATIONS); do
  ...
  git commit -m "feat(frontend): recipe-book multi-select v${i} iteration" --allow-empty 2>/dev/null || true
  ...
  git commit -m "feat(frontend): recipe-book multi-select v${i} eval — score ${score}/100" --allow-empty 2>/dev/null || true
  ...
  if [[ $prev_score -gt 0 && $score -lt $((prev_score - ROLLBACK_THRESHOLD)) ]]; then
    ...
    git commit -m "feat(frontend): recipe-book multi-select v${i} reverted — regression ${prev_score} to ${score}" 2>/dev/null || true
    continue
  fi
...
echo "  Artifact:  solutions/business/recipe-book/frontend/"
echo "  History:   git log --grep='multi-select' --oneline"
echo ""
echo "  Next steps:"
