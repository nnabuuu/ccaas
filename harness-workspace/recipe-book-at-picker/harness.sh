...
# ============================================================
# recipe-book-at-picker Harness — Code Mode Iteration Loop
#
# Integrates @Picker (AtPicker) into recipe-book solution.
# Artifact: solutions/business/recipe-book/{backend,frontend}/
...
# --- Configuration ---
TASK_NAME="recipe-book-at-picker"
MAX_ITERATIONS=6
SCORE_THRESHOLD=90
MIN_IMPROVEMENT=3
...
run_validation() {
  ...
  # Backend tests
  echo "  [Validation] Running backend tests..."
...
这是**第一轮**迭代 (v1)。你需要同时修改后端和前端来集成 @Picker。

**优先级**：先完成后端 Phase 1-2（实体注册 + browse 扩展），再完成前端 Phase 3-4（依赖 + MentionPicker）。
确保每个 Phase 完成后相应的 tsc 通过。

**关键**：
- 后端只修改 referenceable/ 下的文件
- 前端 package.json 添加 context-layer-react 依赖后要 npm install
- MentionPicker 的 baseUrl 必须用 :3002（recipe backend），不是 :3001
- 参考 edu-platform 的 referenceable 实现（多实体注册 + setRelations）
- 参考 MentionPicker.tsx 和 MentionContext.tsx 理解组件 API
...
**读 eval report**: harness-workspace/recipe-book-at-picker/eval-reports/v${prev}-eval.md — 告诉你哪里扣分了、如何修复。
...
echo "║  Artifact: solutions/business/recipe-book/{backend,frontend}║"
echo "║  Frozen:                                                    ║"
...
for i in $(seq $START_VERSION $MAX_ITERATIONS); do
  ...
  if ! run_validation; then
    echo "  Validation FAILED. Reverting changes and skipping to next iteration."
    cd "$REPO_ROOT"
    git checkout -- "solutions/business/recipe-book/backend/src/referenceable/" 2>/dev/null || true
...
echo "  Artifact:  solutions/business/recipe-book/{backend,frontend}/"
echo "  History:   git log --grep='at-picker' --oneline"
echo ""
echo "  Next steps:"
