...
# ============================================================
# live-lesson-v2-dashboard Harness — Code Mode Iteration Loop
#
# Upgrades live-lesson to V2: backend data layer + teacher
# swimlane dashboard + student 4-phase task flow.
# Artifact: solutions/business/live-lesson/{backend,data,frontend}/
...
# --- Configuration ---
TASK_NAME="live-lesson-v2-dashboard"
MAX_ITERATIONS=10
SCORE_THRESHOLD=98
MIN_IMPROVEMENT=3
...
# NOTE: backend/src/ is NOT frozen — it needs changes for V2
FROZEN_DIRS=(
...
run_step() {
  ...
  # Check if step already completed (for --resume sub-step recovery)
...
run_validation() {
  echo "  [Validation] Running backend build + frontend typecheck + build..."
  local errors=0
...
start_lesson_backend() {
  ...
  # ALWAYS rebuild backend (backend/src/ may have changed)
  cd "${SOLUTION_DIR}/backend"
  if [[ ! -d "node_modules" ]]; then
    npm install --legacy-peer-deps --no-audit --no-fund 2>&1 | tail -3
  fi
  ...
  # Re-seed DB after backend rebuild (manifest may have changed)
...
This is **iteration v1** (first round). You need to implement the full V2 upgrade:
backend data layer + teacher swimlane dashboard + student 4-phase task flow.

**Priority**: Phase 1-2 first (backend), then Phase 3-5 (frontend), then Phase 6 (integration).
Each phase must pass its validation check before moving on.

**Key points**:
- Read ALL design reference files (teacher.html, student-app.jsx, colors_and_type.css, etc.)
- Read manifest.json to understand the data structure for answer keys
- Read existing backend code to understand the service structure
- Backend changes: add answerKey to manifest, auto-grading in submit, progress tracking
- Frontend: rewrite TeacherShell (swimlane) + StudentShell (4-phase) with real backend data
- Do NOT break existing /lesson routes
...
**Read eval report**: harness-workspace/live-lesson-v2-dashboard/eval-reports/v${prev}-eval.md — tells you what lost points and how to fix it.
...
echo "║  Artifact: solutions/business/live-lesson/{backend,data,fe}║"
echo "║  Frozen:                                                    ║"
...
echo "║  NOT frozen: backend/src/, data/, frontend/                 ║"
echo "║  Max iterations: ${MAX_ITERATIONS}  |  Threshold: ${SCORE_THRESHOLD}/100              ║"
echo "║  Cost cap: \$${MAX_COST_USD}                                            ║"
echo "╚══════════════════════════════════════════════════════════════╝"

if $RESUME; then
  # Try state.json first for sub-step level resume, fall back to eval file count
  ...
  if [[ -n "$state_iter" && "$state_iter" != "null" && "$state_iter" != "0" ]]; then
    ...
    if [[ "$state_iter_status" == "running" ]]; then
      # Resume mid-iteration — run_step will skip completed sub-steps
...
for i in $(seq $START_VERSION $MAX_ITERATIONS); do
  ...
  # ─── Step 3: Validation (backend build + frontend typecheck + build) ───
  echo ""
  ...
  if ! run_step "$i" "validation" "run_validation"; then
    ...
    cd "$REPO_ROOT"
    git checkout -- "solutions/business/live-lesson/backend/src/" 2>/dev/null || true
    git checkout -- "solutions/business/live-lesson/frontend/src/styles/" 2>/dev/null || true
    git checkout -- "solutions/business/live-lesson/frontend/src/types/reading.ts" 2>/dev/null || true
    git checkout -- "solutions/business/live-lesson/frontend/src/hooks/" 2>/dev/null || true
    ...
    git checkout -- "solutions/business/live-lesson/frontend/src/pages/" 2>/dev/null || true
    git checkout -- "solutions/business/live-lesson/data/" 2>/dev/null || true
  ...
  git commit -m "feat(live-lesson): v2-dashboard v${i} iteration" --allow-empty 2>/dev/null || true
  ...
  git commit -m "feat(live-lesson): v2-dashboard v${i} eval — score ${score}/100" --allow-empty 2>/dev/null || true
  ...
  if [[ $prev_score -gt 0 && $score -lt $((prev_score - ROLLBACK_THRESHOLD)) ]]; then
    ...
    git add \
      ...
      "solutions/business/live-lesson/data/" \
      "solutions/business/live-lesson/frontend/" && \
    git commit -m "feat(live-lesson): v2-dashboard v${i} REVERTED — regression ${prev_score}->${score}" 2>/dev/null || true
    continue
  fi
...
echo "  Artifact:  solutions/business/live-lesson/{backend,data,frontend}/"
echo "  History:   git log --grep='v2-dashboard' --oneline"
echo ""
echo "  Next steps:"
