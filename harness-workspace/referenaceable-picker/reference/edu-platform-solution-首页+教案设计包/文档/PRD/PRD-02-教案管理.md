...
## 7. AI 集成：EntityLevelSession + Referenceable

### 7.1 EntityLevelSession

每个教案实体绑定一个 EntityLevelSession——一个长期存在的对话 session，由 sessionTemplate（如"备课助手"）定义可用的 Skills。

```typescript
interface EntityLevelSession {
  entity_type: 'lesson_plan';
  entity_id: string;                    // 'lp_12345'
  session_id: string;                   // 绑定的 CCaaS session
  session_template_id: string;          // '备课助手' template
  created_at: string;
  // session 本身由 CCaaS core 管理，这里只存绑定关系
}
```

绑定时机：教案创建时自动创建 session 并绑定。session 和教案同生命周期。

教案详情页右侧的 AI panel 就是这个 session 的 UI 表达。所有教案相关的 AI 对话都汇聚到这个 session：用户主动提问、skill_run 产出的追问、@ 引用产生的上下文——最终都在同一个对话中。

### 7.2 Referenceable 抽象

所有可被 @ 引用的实体实现统一的 Referenceable 接口。这是 CCaaS core 层的抽象，solution 层按业务注册具体 type。

```typescript
// @ pill 注入消息时携带的数据（轻量，总在 context window 中）
interface AtReference {
  type: string;           // 'lesson_plan' | 'skill_run' | 'requirement' | 'exercise'
  id: string;
  display_name: string;   // @ pill 显示文本
  summary: string;        // ≤100 字摘要，业务层定义
}

// Agent 需要完整信息时调用的标准 API
// GET /context/entity/{type}/{id}
interface EntityContext {
  ref: AtReference;

  // 结构化字段（每个 type 自定义 schema）
...
教案相关的 Referenceable type 注册：

| type | display_name 示例 | summary 示例 | structured 包含 |
|------|------------------|-------------|----------------|
| `lesson_plan` | 教案:SSS/SAS 新授课 | 八(2)班 数学 新授课 45分钟 学业要求7.3.2 | blocks, requirement_snapshot, exercises |
| `skill_run` | 学情备注:八(2)班SSS/SAS | SSS 错误率 42%，夹角混淆占 43%，连续 3 周下降 | output_data, reasoning_chain, input_params, run_time |
| `requirement` | 课标:7.3.2 全等判定 | 能运用 SSS、SAS 判定三角形全等 | code, text, interpretation, version |
| `exercise` | 练习:SSS/SAS 专项 | 5 道题 含 SSA 辨析 已发布 | questions, difficulty_stats |

Agent 的决策流程：收到 @ 引用 → 先读 summary（已在 context window）→ 如果不够 → 调用 `GET /context/entity/{type}/{id}` 获取 full context → 如果需要关联实体 → 递归获取 relations。

### 7.3 Skill 产出的展示与同步

三层分离：展示层（callout 快照）、推演层（对话）、提交层（apply button）。

**展示层：callout 快照**

Skill 产出以紫色 AI callout block 嵌入教案编辑器。callout 是**快照**——记录某一次 skill_run 的产出，一旦写入不自动变化。

```typescript
interface AICalloutBlock {
  type: 'ai_callout';
  content: string;                      // 展示文本
  source_skill_run_id: string;          // 来源 skill_run
  source_skill_name: string;            // 'Skill: 学情异常检测'
  source_timestamp: string;             // skill_run 执行时间
  freshness: 'fresh' | 'stale';        // 底层数据是否已变化
}
```

时效性检测：当 callout 依赖的底层数据变化时（如新作业提交），`freshness` 变为 `stale`，callout 显示"数据已变化"琥珀标签 + "重新分析"按钮。点击重新分析 → 触发新 skill_run → 新 callout 替换旧的。

**推演层：对话**

用户通过 @ picker 引用 callout → 在 EntityLevelSession 中追问 → 对话中产生新的分析和建议。对话内容不自动改变教案。

**提交层：apply button**

当 Agent 在对话中产出了"可落地到教案的具体建议"时（如修改时间线、补充练习题），Agent 渲染 inline action button：

```typescript
interface ApplyAction {
  type: 'apply_to_entity';
  target_entity: AtReference;           // 要修改的实体
  target_field: string;                 // 要修改的字段，如 'blocks[3].content'
  suggested_value: any;                 // 建议的新值
  description: string;                  // 人类可读描述："将 SSS 讲解从 10 分钟改为 8 分钟"
...
| `pending` | `[应用到教案]`（主按钮） |
| `applied` | `✓ 已应用 · 时间`（灰色文字） |
| `outdated` | `[重新应用]`（对话中又修改了方案） |

点击"应用到教案"→ 调用教案的更新 API → 教案内容变更 → Activity 事件记录 → 按钮变为 ✓。

CCaaS core 提供 `apply_to_entity` action 类型，Agent 自行决定什么时候渲染它。core 不了解教案的业务逻辑——它只知道"Agent 想往某个 entity 的某个 field 写一个值，需要用户确认"。

### 7.4 追问流程（@ picker 驱动）

教案编辑器中 AI callout 的"追问"按钮：

1. 点击 → 右侧 AI panel 输入框自动注入 `@学情备注:八(2)班SSS/SAS` pill
2. 用户在 pill 后写问题 → 发送
3. 消息体包含 AtReference → Agent 解析 → 按需获取 full context
4. Agent 在 EntityLevelSession 上下文中回答
5. 如果回答包含可落地建议 → 渲染 apply button

**不需要 solution 层的特殊注入代码**——@ pill 就是标准的 AtReference，走统一解析。

### 7.5 API

```
# EntityLevelSession 绑定
GET    /api/v1/lesson-plans/{id}/session
       → { session_id, session_template_id }
POST   /api/v1/lesson-plans/{id}/session
       → 创建并绑定 session（教案创建时自动调用）
...
GET    /context/entity-types
       → 所有已注册的 referenceable type 列表

# Apply action
POST   /api/v1/apply-action
       { target_type, target_id, target_field, suggested_value }
       → 执行写入，返回变更记录
```
...
- [ ] 教案创建时自动绑定 EntityLevelSession（备课助手 sessionTemplate）
- [ ] 教案详情页右侧 AI panel 展示 EntityLevelSession 的对话历史和 Skill 产出
- [ ] Skill 产出以 AI callout block 嵌入编辑器，标注来源和时间
- [ ] AI callout 的"追问"按钮自动注入 @ pill 到 AI panel 输入框
- [ ] @ pill 携带 AtReference（type + id + display_name + summary）
- [ ] Agent 可通过 `GET /context/entity/{type}/{id}` 获取完整 EntityContext
- [ ] AI callout 支持时效性检测（fresh/stale）和"重新分析"
- [ ] 对话中可渲染 apply_to_entity action button（应用到教案 / 已应用 / 重新应用）
- [ ] apply action 执行后教案内容更新，Activity 事件记录变更
