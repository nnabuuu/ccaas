...
# ============================================================
# live-lesson-interactive-features Harness — Code Mode Iteration Loop
#
# Implements 4 feature groups on top of the e2e-collaboration pipeline:
#   A: Teacher Direct Step Sync
#   B: Push Notifications
#   C: AI Assistant Free Questions
#   D: Timer & Extend
...
# --- Configuration ---
TASK_NAME="live-lesson-interactive-features"
MAX_ITERATIONS=8
SCORE_THRESHOLD=95
MIN_IMPROVEMENT=3
...
这是**第一轮**迭代 (v1)。你需要实现 SPEC.md 中的 4 个 feature group（A: Step Sync, B: Notifications, C: AI Ask, D: Timer）。

**优先级**：
1. 先实现 backend 3 个新端点 (POST /step, POST /notify, POST /ai/ask) + SSE 命名事件
2. 再实现 frontend useStudentStream hook + 教师端 API 调用
3. 然后接入学生端 SSE 订阅 + 通知 toast + AI 面板调用
4. 最后实现 Timer 倒计时

**关键**：
- 完整阅读现有 backend classroom 源码，理解 SSE 管道后再添加命名事件
- 新的 broadcastNamed() 不能破坏现有的 broadcast()（无名 data 事件）
- useStudentStream 使用 addEventListener 监听命名事件
- AiPanel 需要新 props: lessonId, studentId, step
- Timer 使用 setInterval + stepDuration 计算
...
**读 eval report**: harness-workspace/live-lesson-interactive-features/eval-reports/v${prev}-eval.md — 告诉你哪里扣分了、如何修复。

**策略**：
1. 先做 A/B/C 根因分类
2. 优先修复 A-类（管道断裂）和 B-类（UI 未接入）
3. C-类（样式/体验）最后修
4. 不要重做已经满分的维度
...
# Progress — live-lesson-interactive-features
...
echo "║  Features: Step Sync, Notifications, AI Ask, Timer         ║"
echo "║  Frozen:                                                    ║"
echo "║    packages/                                                ║"
...
for i in $(seq $START_VERSION $MAX_ITERATIONS); do
  ...
  git commit -m "feat(live-lesson): interactive-features v${i} iteration" --allow-empty 2>/dev/null || true
  ...
  git commit -m "feat(live-lesson): interactive-features v${i} eval — score ${score}/100" --allow-empty 2>/dev/null || true
  ...
  if [[ $prev_score -gt 0 && $score -lt $((prev_score - ROLLBACK_THRESHOLD)) ]]; then
    ...
    git commit -m "feat(live-lesson): interactive-features v${i} REVERTED — regression ${prev_score}->${score}" 2>/dev/null || true
    continue
  fi
...
echo "  Artifact:  solutions/business/live-lesson/{frontend,backend}/"
echo "  History:   git log --grep='interactive-features' --oneline"
echo ""
echo "  Next steps:"
