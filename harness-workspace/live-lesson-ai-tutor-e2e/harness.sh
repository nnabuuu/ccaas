...
# ============================================================
# live-lesson-ai-tutor-e2e Harness — Code Mode Iteration Loop
#
# Implements AI tutor pipeline: rich prompts, categorization,
# teacher visibility, student UX.
# Artifact: backend/src/classroom/ + frontend components + manifest
...
# --- Configuration ---
TASK_NAME="live-lesson-ai-tutor-e2e"
MAX_ITERATIONS=10
SCORE_THRESHOLD=98
MIN_IMPROVEMENT=3
...
run_validation() {
  echo "  [Validation] Running backend + frontend build..."
  local errors=0
...
This is **iteration v1** (first round). You need to implement ALL 6 phases:
1. Data layer: aiReferenceQA in manifest, category in QuestionRecord, categoriesMap
2. Backend: rewrite buildAiSystemPrompt (6-layer), parseCategoryFromResponse, update aiAsk
3. API: controller returns { answer, category }
4. Frontend types: ClassroomState.questions add answer + category, useAiAsk returns both
5. Student UX: AiPanel category labels, context-aware chips
6. Teacher UX: category-grouped queue, badges, expandable answers

**Priority**: Backend first (prompt + categorization) → API → Frontend types → UX.

**Key points**:
- Read design documents thoroughly — they are the truth source
- Read existing code before modifying — understand the patterns
- Do NOT modify frozen files
- Run builds to verify after changes
...
**Read eval report**: harness-workspace/live-lesson-ai-tutor-e2e/eval-reports/v${prev}-eval.md — tells you what lost points and how to fix it.
...
echo "║  Artifact: backend classroom + frontend components          ║"
echo "║  Frozen: entities, DTOs, lesson, pages, orchestrator, pkgs  ║"
...
for i in $(seq $START_VERSION $MAX_ITERATIONS); do
  ...
  # ─── Step 3: Validation (backend + frontend build) ───
  echo ""
  ...
  if ! run_step "$i" "validation" "run_validation"; then
    ...
    append_progress $i "FAIL" "Validation failed" "build error"
    continue
  fi
  ...
  git commit -m "feat(live-lesson): ai-tutor-e2e v${i} iteration" --allow-empty 2>/dev/null || true
  ...
  git commit -m "feat(live-lesson): ai-tutor-e2e v${i} eval — score ${score}/100" --allow-empty 2>/dev/null || true
  ...
  if [[ $prev_score -gt 0 && $score -lt $((prev_score - ROLLBACK_THRESHOLD)) ]]; then
    ...
    git commit -m "feat(live-lesson): ai-tutor-e2e v${i} REVERTED — regression ${prev_score}->${score}" 2>/dev/null || true
    continue
  fi
...
echo "  Artifact:  backend/src/classroom/ + frontend components"
echo "  History:   git log --grep='ai-tutor-e2e' --oneline"
echo ""
echo "  Next steps:"
echo "    cat ${PROGRESS_FILE}"
echo "    bash ${HARNESS_DIR}/tests/test-categorization.sh"
echo "    bash ${HARNESS_DIR}/tests/test-prompt-quality.sh"
echo "    bash ${HARNESS_DIR}/tests/test-teacher-api.sh"
echo ""
