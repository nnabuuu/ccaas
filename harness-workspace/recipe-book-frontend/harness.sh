...
# ============================================================
# recipe-book-frontend Harness — Code Mode Iteration Loop
#
# Builds React frontend for recipe-book solution.
...
# --- Configuration ---
TASK_NAME="recipe-book-frontend"
MAX_ITERATIONS=8
SCORE_THRESHOLD=95
MIN_IMPROVEMENT=3
...
这是**第一轮**迭代 (v1)。前端目录尚不存在，你需要从头构建。

**优先级**：先完成 Phase 1-3（脚手架 + 设计系统 + 布局），再完成 Phase 4-7（页面 + 路由）。
确保每个 Phase 完成后 tsc 通过。

**关键**：
- 直接复制 edu-platform 的 design-tokens.css
- 参考 edu-platform 的 Sidebar/TopNav/App.tsx 模式
- config.ts 中 RECIPE_BACKEND_URL = :3002, CCAAS_URL = :3001
- package.json 中 @kedge-agentic/* 使用 file: 链接
...
**读 eval report**: harness-workspace/recipe-book-frontend/eval-reports/v${prev}-eval.md — 告诉你哪里扣分了、如何修复。
...
for i in $(seq $START_VERSION $MAX_ITERATIONS); do
  ...
  git commit -m "feat(frontend): recipe-book frontend v${i} iteration" --allow-empty 2>/dev/null || true
  ...
  git commit -m "feat(frontend): recipe-book frontend v${i} eval — score ${score}/100" --allow-empty 2>/dev/null || true
  ...
  if [[ $prev_score -gt 0 && $score -lt $((prev_score - ROLLBACK_THRESHOLD)) ]]; then
    ...
    git commit -m "feat(frontend): recipe-book frontend v${i} REVERTED — regression ${prev_score}->${score}" 2>/dev/null || true
    continue
  fi
...
echo "  Artifact:  solutions/business/recipe-book/frontend/"
echo "  History:   git log --grep='recipe-book.*frontend' --oneline"
echo ""
echo "  Next steps:"
