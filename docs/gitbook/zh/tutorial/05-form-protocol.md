...
`output_update` 协议是连接 AI Agent 输出与前端表单状态的桥梁。当 Agent 调用 `write_output` MCP 工具时，CCAAS 后端会发出 `output_update` SSE 事件，前端解析后以 SyncCard 形式呈现给用户审批。本章讲解 `write_output` 的工作原理、事件结构（包括嵌套的 `payload.data` 格式），以及生产环境中使用的 SyncCard 审批模式。
...
│ 3. 发出 output_update SSE 事件，数据放在 payload.data 中               │
└─────────────────────┬───────────────────────────────────────────────┘
                      │
┌─────────────────────▼───────────────────────────────────────────────┐
│ react-sdk (useAgentChat)                                            │
│                                                                     │
│ 1. 在 SSE 流上监听 output_update                                     │
...
当 CCAAS 后端接收到 `write_output` 工具结果时，EventMapper 将其包装为 `output_update` SSE 事件。`@kedge-agentic/common` 包定义了 Schema：
...
4. 使用浏览器 DevTools 的 Network 标签检查 SSE 流中的 `output_update`
