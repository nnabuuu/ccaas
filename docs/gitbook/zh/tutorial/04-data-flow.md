...
本章涵盖完整的数据流架构、SSE 事件系统以及你在 Solution 中使用的 React SDK hooks。
...
CCAAS 使用**直连架构**，你的 Solution 前端通过 SSE（Server-Sent Events）直接连接到 CCAAS 后端。CCAAS 后端管理 AI Agent 进程，并将事件实时流式传输回前端。
...
└──────────────┘         └──────────────────┘◄────────►└──────────────┘
  ...
  @kedge-agentic/react-sdk         技能路由                      OpenCode / 等
                           事件流
                           认证鉴权
...
- **消息通过 REST 发送**（`POST /api/v1/sessions/:sessionId/messages`，返回 SSE 流），**响应通过 SSE** 事件流式返回
...
`useAgentChat` hook 通过 REST 端点发送消息，响应以 SSE 流式返回：
...
// POST 返回 text/event-stream（SSE 流）
...
POST 请求返回 SSE 流，事件直接在响应中流式传输。

### 第 3 步：CCAAS 后端处理请求

后端（`SessionsController.sendMessage`）执行以下操作：

1. **SSE 流创建** -- 建立 SSE 响应流
...
当 AI Agent 工作时，CCAAS 后端解析其 stdout 并通过 SSE 流向已连接的客户端发送结构化事件：
...
SDK hooks 自动监听这些 SSE 事件并更新 React 状态。你不需要手动编写事件监听器。

## React SDK Hooks

`@kedge-agentic/react-sdk` 包提供五个核心 hooks，共同管理完整的数据流。以下是它们在真实 Solution 中的组合方式：
...
| 属性 | 类型 | 用途 |
|------|------|------|
| `socket` | `Socket \| null` | 底层连接实例（SSE 模式下可能为 null） |
...
| `sendMessage(content, options?)` | function | 发送消息（REST + SSE 响应） |
...
CCAAS 后端通过 SSE 流发出多个类别的事件。理解每个类别对构建响应式前端至关重要。注意 SDK hooks 会自动处理所有这些事件 -- 这里的参考是为了帮助你理解底层发生了什么。
...
└─────────┼─────────────────┼────────────────┼───────────────┼──────────┘
     ...
     │                      CCAAS 后端 (:3001)                           │
     │                                                                   │
     │  POST /sessions/:id/messages           SSE 事件流                  │
     │  GET  /sessions/:id/events            推送通道                    │
...
- **出站**：REST `POST /api/v1/sessions/:sessionId/messages`（返回 SSE 流）
- **入站**：SSE 事件（`text_delta`、`output_update`、`agent_status`、`tool_activity` 等）
- **用途**：实时 AI Agent 交互
...
1. **CCAAS 使用直连架构** -- Solution 前端通过 SSE 直接连接 CCAAS 后端，不需要通过 Solution 后端中继
2. **消息通过 REST 发送，响应通过 SSE 流式返回** -- `POST /sessions/:id/messages` 发送消息并返回 SSE 流
