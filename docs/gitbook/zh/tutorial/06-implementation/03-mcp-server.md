...
在本节中，你将为任务管理器 Solution 构建 MCP Server。MCP Server 提供 AI Agent 在对话过程中可以调用的工具——最重要的是 `write_output` 工具，它将 AI 生成的数据同步到前端表单。
...
const fieldSchemas: Record<SyncField, z.ZodType> = {
  taskTitle: z.string().min(1, '标题不能为空').max(200),
  taskDescription: z.string().max(5000),
  priority: z.enum(['low', 'medium', 'high', 'urgent']),
  status: z.enum(['todo', 'in_progress', 'done', 'cancelled']),
  projectId: z.string().uuid('项目 ID 必须是有效的 UUID'),
  dueDate: z.string().regex(
    /^\d{4}-\d{2}-\d{2}$/,
    '截止日期必须是 YYYY-MM-DD 格式'
  ),
...
const writeOutputTool: Tool = {
  name: 'write_output',
  description: `将任务数据写入前端表单。每次更新一个字段。

有效字段: ${SYNC_FIELDS.join(', ')}

字段类型:
- taskTitle: string (任务标题，1-200 字符)
- taskDescription: string (任务描述，最多 5000 字符)
- priority: "low" | "medium" | "high" | "urgent"
- status: "todo" | "in_progress" | "done" | "cancelled"
- projectId: string (项目的 UUID)
- dueDate: string (ISO 日期格式，例如 "2026-03-15")
- tags: string[] (标签数组，例如 ["frontend", "urgent"])
...
async function main() {
  ...
  console.error('Task Manager MCP Server 已启动');
}
...
echo '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"write_output","arguments":{"field":"taskTitle","value":"审查 API 文档","preview":"设置标题"}}}' | node dist/index.js
...
- **有效调用：** `{ "data": { "field": "taskTitle", "value": "审查 API 文档" }, "status": "success" }`
...
MCP Server 返回:
{
  "data": { "field": "taskTitle", "value": "审查 API 文档" },
  "status": "success"
}
          │
          ▼
...
除了 `write_output`，你还可以添加领域特定的工具。对于任务管理器，你可能会添加一个搜索现有任务的工具：

```typescript
const searchTasksTool: Tool = {
  name: 'search_tasks',
  description: '按关键词、状态或项目搜索现有任务',
  inputSchema: {
    type: 'object',
    properties: {
      query: {
...
AI Agent 可以使用这个工具在创建新任务之前检查是否有重复的任务，或者了解当前的项目上下文。
...
- [ ] 无效的值（如 `priority: "critical"`）返回验证错误
...
如果你的 MCP Server 验证 `taskTitle` 但 Skill 指令中写的是 `title`，AI Agent 会发送 `title` 然后得到验证错误。将 `SYNC_FIELDS` 数组作为唯一数据源，并在 MCP Server 和 Skill 中都引用它。
