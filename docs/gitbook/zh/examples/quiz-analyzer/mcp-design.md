...
| 关注点 | SQLite | JSON + Map |
|--------|--------|------------|
| 启动速度 | 数据库连接 + Schema 初始化 | 逐文件 JSON 解析 |
| 部署 | DB 文件 + 迁移 | 21 个学科 JSON 文件，无需迁移 |
| 查询速度 | 每次请求一次 SQL 查询 | O(1) Map 查找 |
| 叶节点检测 | 运行时查询子节点 | O(1) — 预填充 `children[]` |

31,497 个知识点约占 8 MB 内存。它们存储于 `data/subjects/` 目录下的 21 个学科文件中——每个学科一个文件，对应一个学段（小学、初中或高中）。Loader 在启动时读取全部 21 个文件，合并为单一内存数组。对于这个数据集规模，只读查询场景下内存方案严格优于数据库。
