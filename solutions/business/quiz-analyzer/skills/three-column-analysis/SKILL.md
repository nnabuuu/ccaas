...
**所有题目首选 Mode C**（`search_knowledge_points_by_priority`）。
模式选择的核心变量是 `limitPerKeyword`，而非工具本身：

| 题目类型 | limitPerKeyword | 原因 |
|---------|----------------|------|
| **理科题**（数学/物理/化学/生物）| `5`（默认） | KP 命名精确，少量结果已足够 |
| **文科题**（历史/地理/政治/语文）| `10–15` | KP 之间有概念交叉，需更多候选才能正确区分 |
| **3+ 并列概念的综合题** | `5`，完成后检查覆盖；若缺口明显 → 追加 Mode A（`batch_search_knowledge_points`） | Mode A 的 `matchScore`（命中关键词数 × 10 + level）能将多概念命中节点浮出水面 |

**Mode B 触发条件（满足任一才切换，针对具体的未命中关键词）**

- ① `coveredKeywords` 缺少核心概念
- ② `leafOnly` 过滤后结果为空（叶节点全被滤掉）
- ③ 所有返回结果 `isLeaf: false`（全是父节点）
- ④ 首轮搜索 `coverageScore = 0`
...
> ⚠️ **必执行**：每次 Mode C/A/B 搜索完成、候选 KP 集确定后，在调用 `write_output` 之前执行此节。

**Step A: fullName 学科路径确认（所有题目）**

```
对候选集中每个 KP：
  - 阅读其 fullName（如 "初中知识点 > 图形与几何 > 三角形 > 勾股定理 > 勾股定理的实际应用"）
  - 确认：学科路径是否与题目一致？（数学题的 KP 不应来自"语文"分支）
  - 不一致 → 丢弃
```
...
```
从题干（不是选项/答案）列出 2–4 个"本题要求学生掌握的知识维度"：
  维度 ≠ 关键词，是更抽象的"这道题考什么"
  例（二次函数最小值题）: [二次函数图像性质, 顶点/最值求法]

对每个维度，找候选集中对应的 KP（看 fullName）：
  → ✅ 已覆盖（有对应 KP）
  → ❌ 缺口（无对应 KP）
```

**Step C: 文科歧义消解（仅当多个候选 KP fullName 高度相似时）**

```
触发条件：多个候选 KP 名字相近或共享同一父路径

处理方式：
  1. 列出所有相似候选 KP 的完整 fullName
  2. 阅读题目的具体问法（题目问的是：原因？经过？意义？影响？）
  3. 匹配：选取 fullName 中含有题目问法对应概念的那个叶节点
  4. 其余候选保留但置信度降低（0.5–0.7），标注为"备选"

例：题目"五四运动的导火索是什么" → 选"五四运动的导火索"叶节点（若存在），
    而非"五四运动的历史意义"或"五四运动的国际背景"
```

**Step D: 迭代填补缺口（最多 2 轮）**

```
对每个未覆盖维度：
  1. 换一种说法提取新关键词（不要重复已失败的词）
  2. Mode C 重搜（文科 limitPerKeyword=10–15）；若仍无结果 → Mode B 下钻
  3. 将新找到的叶节点加入候选集
  4. 第 2 轮仍未找到 → 接受缺口，置信度标注 ≤ 0.3
...
| 工具名称 | 适用场景 | 数据源 |
|---------|---------|--------|
| `search_knowledge_points_by_priority` | **Mode C 主路径**：单/双核心概念，按优先级迭代；理科默认 `limitPerKeyword=5`，文科用 `10–15` | JSON |
| `batch_search_knowledge_points` | **Mode A**：3+ 并列概念的综合题，利用多关键词 `matchScore`（命中关键词数 × 10 + level）将多概念命中节点浮出水面 | JSON |
| `list_root_knowledge_points` | **Mode B Step 1–2**：关键词搜索失败时，从顶层分类开始层级下钻 | JSON |
| `get_knowledge_point_children` | **Mode B Step 3**：逐层展开子节点（含 `isLeaf` 信息） | JSON |
| `search_knowledge_points_under` | **Mode B Step 4**：子节点过多时，在选定子树内缩小关键词范围 | JSON |
| `get_knowledge_point_path` | **Mode B 确认步骤**：获取完整路径；Mode A/C 已内联 `fullName`，无需重复调用 | JSON |
