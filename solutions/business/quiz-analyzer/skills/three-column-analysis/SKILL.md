...
2. `batch_search_knowledge_points` - 标注知识点，Mode A（依赖题目内容）；有父节点时切换 Mode B 层级遍历
...
优先使用 **JSON 数据源** (更快):
- ✅ `batch_search_knowledge_points` (Mode A，批量搜索，leafOnly: true)
- ✅ `list_subjects` (科目搜索，JSON)
- ✅ `list_root_knowledge_points` / `get_knowledge_point_children` / `search_knowledge_points_under` (Mode B 层级遍历，JSON)
- ⚠️ `search_knowledge_points_json` / `search_knowledge_points` (仅在以上工具失败时使用)
...
| 情况 | 选用模式 |
|------|---------|
| 题目关键词高度明确（如"求对称轴"） | Mode A（快速批量搜索） |
| batch_search 返回结果中含有父节点 | Mode B（层级遍历） |
| 题目涉及多个知识域（如"函数+几何"） | Mode A + Mode B 各自搜索 |
| batch_search 返回空或仅 1 个结果 | Mode B 兜底 |
...
3. 若所有结果均为叶节点（isLeaf: true）→ 直接进入确认步骤
4. 若有父节点混入（isLeaf: false）→ 切换到 Mode B 精准定位
```
...
Step 5: 确认与标注
  ...
  get_knowledge_point_path(id) → 构建面包屑路径
...
| `batch_search_knowledge_points` | 批量关键词搜索知识点（Mode A） | JSON |
...
| `get_knowledge_point_path` | 获取知识点完整路径 | 确认步骤：构建面包屑 |

### 可选工具

| 工具名称 | 用途 | 使用场景 |
|---------|------|----------|
| `search_knowledge_points_json` | 单关键词搜索知识点 | 已被 batch_search 替代，仅兼容备用 |
...
2. **调用** `batch_search_knowledge_points`（Mode A）
   - 输入: `{ keywords: ["二次函数", "最值"], leafOnly: true }`
   - 输出: `[{ id, name, isLeaf, ... }]`（若有父节点 → 切换 Mode B 层级遍历）
