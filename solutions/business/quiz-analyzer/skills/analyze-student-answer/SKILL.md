...
**实例（平方差公式）：**
- Mode C 搜索 ["平方差公式", "因式分解"]（leafOnly: true）→ 已返回叶节点 ✅
- 若核心概念未命中（coveredKeywords 缺失）→ 切换 Mode B：
  `get_knowledge_point_children("因式分解 id")`
...
### 步骤 7: 使用 write_output 发送分析结果

**重要**：分析完成后，调用 `write_output` 将 StudentAnswer 对象发送到前端。

```json
{
  "field": "studentAnswer",
  "value": { /* 步骤6生成的 StudentAnswer 对象 */ },
  "preview": "学生答案分析完成"
}
```

前端将显示"同步到表单"按钮，用户确认后应用结果。
...
### 必用工具
1. `get_quiz_details` - 获取题目完整信息（题干、标准答案、解题步骤、知识点）
2. `write_output` - 保存分析结果到前端

### 知识点搜索工具
3. `search_knowledge_points_by_priority` - 按优先级迭代搜索（Mode C，推荐首选）
4. `batch_search_knowledge_points` - 多关键词批量搜索（Mode A，含 fullName）
5. `list_subjects` - 搜索科目（Mode B Step 1）
6. `list_root_knowledge_points` - 获取根节点（Mode B Step 2）
7. `get_knowledge_point_children` - 获取子节点（Mode B 逐层展开）
8. `search_knowledge_points_under` - 在子树内搜索（Mode B 子节点过多时）
9. `get_knowledge_point_path` - 获取知识点完整路径（Mode B 确认步骤）
10. `verify_knowledge_point_tags` - 验证知识点 ID 存在（含 fullName 和建议）
