...
# Custom initialization - Excel import workflow
custom_init() {
    log_step "3.5" "Excel data import workflow"

    # Check if Excel files exist
    if [ ! -f "$SCRIPT_DIR/resources/目录信息.xlsx" ] || \
       [ ! -f "$SCRIPT_DIR/resources/知识点信息.xlsx" ] || \
       [ ! -f "$SCRIPT_DIR/resources/题目信息.xlsx" ]; then
        log_warn "Excel files not found in resources/"
    ...
    if [ -d "$scripts_dir" ]; then
        ...
        log_success "Script dependencies installed"

        # Analyze Excel structure
        if [ -f "$SCRIPT_DIR/resources/目录信息.xlsx" ]; then
            log_info "Analyzing Excel structure..."
            node analyze-excel-structure.js > ../data/excel-analysis.txt 2>&1
            log_success "Excel structure analyzed (see data/excel-analysis.txt)"
        fi

        # Import Excel to database
        if [ -f "$SCRIPT_DIR/resources/目录信息.xlsx" ]; then
            log_info "Importing Excel data to SQLite..."
            node import-excel-to-db.js
            log_success "Excel data imported"
        else
            log_warn "Skipping Excel import (files not found)"
            log_info "Creating empty database with schema..."
            cd "$SCRIPT_DIR"
            mkdir -p data
        ...
        if [ -f "data/quiz-analyzer.db" ]; then
            ...
            local subjects_count=$(sqlite3 data/quiz-analyzer.db "SELECT COUNT(*) FROM subjects;")
            local kp_count=$(sqlite3 data/quiz-analyzer.db "SELECT COUNT(*) FROM knowledge_points;")
            local quiz_count=$(sqlite3 data/quiz-analyzer.db "SELECT COUNT(*) FROM quizzes;")
            echo "  Subjects: $subjects_count"
            echo "  Knowledge Points: $kp_count"
            echo "  Quizzes: $quiz_count"
            log_success "Database ready"
        fi
    else
        log_warn "Scripts directory not found, skipping Excel import"
    fi
...
main() {
    ...
    # Step 3.5: Custom initialization (Excel import + MCP build)
    custom_init
