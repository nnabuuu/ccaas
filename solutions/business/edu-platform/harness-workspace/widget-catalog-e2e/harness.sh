...
# ─────────────────────────────────────────────────────────
# Widget Catalog E2E — Harness Orchestrator
# ─────────────────────────────────────────────────────────
# Usage:
...
EDU_MCP="$SOLUTION_ROOT/mcp-server"
...
COST_PER_ITERATION=10.50  # rough estimate in USD
...
log "=== Widget Catalog E2E Harness ==="
...
if $DRY_RUN; then
  ...
  log "[DRY RUN] Generator: claude -p → creates/modifies widget code"
  log "[DRY RUN] Pre-gate: tsc --noEmit (frontend + mcp-server)"
  log "[DRY RUN] Evaluator: claude -p → reads code + screenshots → scores"
...
3. (第一轮，无上轮 eval report)"
  ...
  GENERATOR_PROMPT="你是 Widget Catalog E2E 的 Generator agent。这是第 $VERSION 轮迭代。
...
4. 读参考设计 HTML:
   - $SOLUTION_ROOT/reference/chat-interface-details/metric-dashboard.html
   - $SOLUTION_ROOT/reference/chat-interface-details/step-wizard.html
   - $SOLUTION_ROOT/reference/chat-interface-details/review-panel.html
   - $SOLUTION_ROOT/reference/chat-interface-details/session-input-suggestions.html
5. 读现有代码:
   - $EDU_FRONTEND/src/App.tsx
   - $EDU_FRONTEND/src/widget-registry.ts (如果存在)
   - $EDU_FRONTEND/src/widgets/ (如果存在)
   - $EDU_MCP/src/index.ts
6. 读 builtin widget 代码（参考模式，不要修改）:
   - $CHAT_PKG/src/widgets/components/MetricDashboard.tsx
   - $CHAT_PKG/src/widgets/components/StepWizard.tsx
   - $CHAT_PKG/src/widgets/components/ReviewPanel.tsx
   - $CHAT_PKG/src/types/widget.ts
   - $CHAT_PKG/src/context/ChatInterfaceContext.tsx
7. 读 W4 组件（可修改）:
   - $CHAT_PKG/src/components/SessionContextBar.tsx
   - $CHAT_PKG/src/components/QuickSuggestions.tsx
   - $CHAT_PKG/src/components/chat/ChatInterfaceComposer.tsx

然后：
8. 制定本轮改进计划（优先修复 eval 中扣分最多的项）
9. 修改代码
10. 运行验证:
    cd $EDU_FRONTEND && npx tsc --noEmit
    cd $EDU_MCP && npx tsc --noEmit
...
完整指南: $HARNESS_DIR/prompts/generator.md"
  ...
  if $TOOL_PASS && ! (cd "$EDU_MCP" && npx tsc --noEmit 2>>"$HARNESS_DIR/eval-reports/v${VERSION}-tsc-errors.txt"); then
    err "[v$VERSION] MCP Server tsc --noEmit FAILED"
    TOOL_PASS=false
  fi

  if ! $TOOL_PASS; then
    log "[v$VERSION] tsc failed — scoring as 0, reverting widget changes."
  ...
  (cd "$PROJECT_ROOT" && \
    git add solutions/business/edu-platform/frontend/src/widgets/ \
           solutions/business/edu-platform/frontend/src/widget-registry.ts \
           solutions/business/edu-platform/frontend/src/App.tsx \
           solutions/business/edu-platform/mcp-server/src/index.ts \
           packages/chat-interface/src/components/SessionContextBar.tsx \
           ...
           packages/chat-interface/tailwind.config.js \
           2>/dev/null; \
    git commit -m "feat(edu-platform): widget-catalog-e2e harness v$VERSION iteration" -q 2>/dev/null) || \
  ...
  if check_port "$EDU_FRONTEND_PORT" && check_port "$CORE_BACKEND_PORT" && check_port "$EDU_BACKEND_PORT"; then
    ...
    log "[v$VERSION] Pre-flight: Not all ports reachable (frontend=$EDU_FRONTEND_PORT, core=$CORE_BACKEND_PORT, edu=$EDU_BACKEND_PORT) — browser verification DISABLED"
  fi
  ...
  if $BROWSER_VERIFY_AVAILABLE; then
    BROWSER_CONTEXT="
## 浏览器验证状态
- **ENABLED** — 所有服务已启动，MUST perform browser verification for D5 scoring.
- Frontend: http://localhost:$EDU_FRONTEND_PORT
- Core Backend: http://localhost:$CORE_BACKEND_PORT
- Edu Backend: http://localhost:$EDU_BACKEND_PORT"
    ...
    BROWSER_CONTEXT="
## 浏览器验证状态
- **DISABLED** — 部分服务未启动。D5 max 1/5 due to inability to verify runtime rendering."
  fi

  # ─── Step 3: Evaluator ───
  log "[v$VERSION] Running Evaluator agent..."

  EVALUATOR_PROMPT="你是 Widget Catalog E2E 质量的独立 Evaluator agent。这是第 $VERSION 轮迭代的评估。
${EDU_CONTEXT}
${BROWSER_CONTEXT}
...
3. 读 widget 代码: $EDU_FRONTEND/src/widgets/ 目录下所有文件
4. 读 registry: $EDU_FRONTEND/src/widget-registry.ts
5. 读 App.tsx: $EDU_FRONTEND/src/App.tsx
6. 读 MCP Server: $EDU_MCP/src/index.ts
7. 读 W4 组件:
   - $CHAT_PKG/src/components/SessionContextBar.tsx
   - $CHAT_PKG/src/components/QuickSuggestions.tsx
   - $CHAT_PKG/src/components/chat/ChatInterfaceComposer.tsx
8. 读参考设计:
   ...
   - $SOLUTION_ROOT/reference/chat-interface-details/review-panel.html
   - $SOLUTION_ROOT/reference/chat-interface-details/session-input-suggestions.html
9. 运行代码分析 + frozen file 检查
10. 逐维度评分 + penalty 检查
11. 汇总分数，输出 eval report
...
完整的 Evaluator 指南在: $HARNESS_DIR/prompts/evaluator.md"
  ...
  D1=$(grep -oE 'D1 MetricDashboard.*[0-9]/5' "$EVAL_REPORT" | grep -oE '[0-9]/5' | head -1 || echo "?")
  D2=$(grep -oE 'D2 StepWizard.*[0-9]/5' "$EVAL_REPORT" | grep -oE '[0-9]/5' | head -1 || echo "?")
  D3=$(grep -oE 'D3 ReviewPanel.*[0-9]/5' "$EVAL_REPORT" | grep -oE '[0-9]/5' | head -1 || echo "?")
  D4=$(grep -oE 'D4 Session.*[0-9]/5' "$EVAL_REPORT" | grep -oE '[0-9]/5' | head -1 || echo "?")
  D5=$(grep -oE 'D5 E2E.*[0-9]/5' "$EVAL_REPORT" | grep -oE '[0-9]/5' | head -1 || echo "?")
  D6=$(grep -oE 'D6.*质量.*[0-9]/5\|D6.*Code.*[0-9]/5' "$EVAL_REPORT" | grep -oE '[0-9]/5' | head -1 || echo "?")
  ...
  (cd "$HARNESS_DIR" && \
    ...
    git commit -m "docs(edu-platform): widget-catalog-e2e harness v$VERSION eval score $SCORE" -q 2>/dev/null) || true
...
log "  git log --oneline --grep='widget-catalog-e2e harness'"
