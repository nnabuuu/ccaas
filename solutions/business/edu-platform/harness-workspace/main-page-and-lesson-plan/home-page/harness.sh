...
**Your starting point**: Frontend has only ChatInterface + LoginPage. No routing, no HomePage, no TopNav. Create:
1. Install \`react-router-dom\`
2. Create \`frontend/src/styles/design-tokens.css\` with all CSS variables
3. Add BrowserRouter to \`main.tsx\`, restructure \`App.tsx\` with Routes
4. Create TopNav component
5. Create HomePage + HeroSection (static first)
6. Create \`types/dashboard.ts\`

**Priority**: Get \`npx tsc --noEmit\` to pass first. Then routing. Then components.
...
**Read the eval report**: \`harness-workspace/main-page-and-lesson-plan/home-page/eval-reports/v${prev}-eval.md\` — it tells you exactly what to fix.
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  ${TASK_NAME} Harness — Frontend 首页 + 全局导航              ║"
echo "║  Artifact: frontend/src/ (Sidebar+TopNav+HomePage+5 components)║"
echo "║  Dimensions: D1-D5 (Visual/Component/Data/Routing/Code)      ║"
...
for i in $(seq $START_VERSION $MAX_ITERATIONS); do
  ...
  git commit -m "feat(frontend): home-page v${i} iteration" --allow-empty 2>/dev/null || true
  ...
  git commit -m "feat(frontend): home-page v${i} eval — score ${score}/100" --allow-empty 2>/dev/null || true
  ...
  if [[ $prev_score -gt 0 && $score -lt $((prev_score - ROLLBACK_THRESHOLD)) ]]; then
    ...
    cd "$REPO_ROOT"
    git checkout HEAD~2 -- \
      "solutions/business/edu-platform/frontend/src/styles/" \
      ...
      "solutions/business/edu-platform/frontend/src/types/dashboard.ts" \
      "solutions/business/edu-platform/frontend/src/App.tsx" \
      "solutions/business/edu-platform/frontend/src/main.tsx" 2>/dev/null || true
    git add "solutions/business/edu-platform/frontend/" && \
    git commit -m "feat(frontend): home-page v${i} REVERTED — regression ${prev_score}->${score}" 2>/dev/null || true
    continue
  fi
...
echo "  History:   git log --grep='home-page' --oneline"
echo ""
echo "  Next steps:"
echo "    cat ${PROGRESS_FILE}"
echo "    cd solutions/business/edu-platform/frontend && npx tsc --noEmit"
echo "    npm run dev  # then open http://localhost:5173/"
echo ""
