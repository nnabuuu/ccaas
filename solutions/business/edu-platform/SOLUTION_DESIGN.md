...
中小学教师在备课、出题、学情分析、调课四个高频场景中面临以下痛点：

- **备课耗时**：手动查进度、翻教材、写教案，一节课备课 2-3 小时
- **出题低效**：难以精准匹配知识点和难度，缺乏学情数据支撑
- **学情分散**：数据散落在多个系统中，缺乏整合分析和可视化
- **调课繁琐**：互换、代课、改时需多方沟通，冲突检测靠人工、容易出错
- **工具割裂**：备课、出题、学情分析、调课使用不同工具，无法联动

Edu Platform 的目标是将这四个场景整合到一个对话界面中，通过 AI Agent 自动获取上下文、呈现结构化数据、驱动跨 Skill 协同工作流。
...
**决策**：4 个 Skill 通过 `suggest_actions` 的 `skill_hint` 字段实现跨 Skill 引导。
...
- **调课助手**：独立调课流程（互换/代课/改时/补课），通过 timetable MCP 工具完成
...
| `skills/reschedule-class/SKILL.md` | 调课助手 prompt |

### 工具层
| 文件 | 用途 |
|------|------|
| `mcp-server/src/index.ts` | MCP Server 入口（14 个已注册工具 + handler） |
...
| 未经确认直接提交调课申请 | 必须 show_info_card + suggest_actions 确认门控 |
| teacherId 使用字面量 "current" | 必须从 sessionContext 获取实际教师 ID |
