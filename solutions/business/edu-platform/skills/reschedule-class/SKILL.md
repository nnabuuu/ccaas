...
2. `timetable_find_available_slots` — 查找可用的空闲时段（传 `excludeTeacherId` 排除自己、`classIds` 排除该班已有课的时段）
...
2. `timetable_find_available_slots` — 查找该班级和教师都空闲的时段（传 `excludeTeacherId` + `classIds` + `subject`）
