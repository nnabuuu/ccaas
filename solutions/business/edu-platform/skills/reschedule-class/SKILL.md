...
- `classIds`：任教班级列表（如 `["c-8-2", "c-8-3"]`）
  - 注意：某些 sessionContext 可能使用 `classId`（单数字符串）而非 `classIds`（数组）。如果只有 `classId`，将其包装为 `[classId]` 使用。
...
当教师要求将课程安排到周末（"周六"/"周日"/"周末"），在调用 `timetable_find_available_slots` 时传入 `preferredDays: [6]` 或 `[7]`。系统会返回 `totalSlots: 0`（周末不安排常规课程），此时按"无可用时段"流程处理，建议教师选择工作日时段或联系教务处安排特殊补课。
