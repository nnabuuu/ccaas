      ...
      {steps.map((label, i) => (
        <button
          ...
          className={`flex-1 text-center py-2 text-[11px] border-b-[2.5px] transition-colors cursor-pointer ${
            i === current
              ? 'text-[var(--ck-t1)] font-semibold border-[var(--ck-t1)]'
              : i < current
                ? 'text-[var(--ck-success-t)] border-[var(--ck-success-t)]'
                : 'text-[var(--ck-t3)] border-[var(--ck-b2)] cursor-default'
          }`}
        >
          {i < current ? '\u2713 ' : ''}
          {label}
  ...
  return (
    <div>
          ...
          <div key={ri} className="flex gap-2.5 mb-3">
            {r.map((f) => (
              <div key={f.key} className="flex-1">
                <div className="text-[11px] text-[var(--ck-t2)] mb-1 font-medium">{f.label}</div>
                <select
                  className="w-full px-2.5 py-2 border-[0.5px] border-[var(--ck-b1)] rounded-md text-[12px] bg-[var(--ck-bg1)] text-[var(--ck-t1)] outline-none focus:border-[var(--ck-info-t)]"
      ...
      <div className="flex justify-end mt-3.5">
        <button
          onClick={onNext}
          className="text-[12px] px-4 py-[7px] rounded-[var(--ck-r)] bg-[var(--ck-t1)] text-[var(--ck-bg1)] border-[0.5px] border-[var(--ck-t1)] cursor-pointer transition-colors hover:opacity-90 active:scale-[0.98]"
        >
          {'\u2192'} 下一步
        </button>
      </div>
...
function TreePanel({
  ...
  return (
    <div>
      <div className="border-[0.5px] border-[var(--ck-b1)] rounded-[var(--ck-rl)] px-3.5 py-2.5 max-h-[200px] overflow-y-auto text-[12px] text-[var(--ck-t2)]">
        {items.map((chapter) => (
          <div key={chapter.id}>
            <div
              className="py-[3px] font-medium cursor-pointer hover:text-[var(--ck-t1)]"
                ...
                return (
                  <div
                    ...
                    className={`pl-[18px] py-[3px] cursor-pointer ${
                      isOn ? 'text-[var(--ck-t1)] font-semibold' : ''
                    }`}
      ...
      {selected.length > 0 && (
        <div className="text-[11px] text-[var(--ck-t3)] mt-1.5">
          已选: {selected.join(', ')}
        </div>
      )}
      <div className="flex gap-2 justify-end mt-3.5">
        <button
          onClick={onPrev}
          className="text-[12px] px-4 py-[7px] rounded-[var(--ck-r)] border-[0.5px] border-[var(--ck-b1)] bg-[var(--ck-bg1)] text-[var(--ck-t2)] cursor-pointer hover:bg-[var(--ck-bg2)] transition-colors"
        >
          {'\u2190'} 上一步
        </button>
        <button
          ...
          className="text-[12px] px-4 py-[7px] rounded-[var(--ck-r)] bg-[var(--ck-t1)] text-[var(--ck-bg1)] border-[0.5px] border-[var(--ck-t1)] cursor-pointer transition-colors hover:opacity-90 active:scale-[0.98] disabled:opacity-35 disabled:cursor-not-allowed"
        >
          查看学情 {'\u2192'}
        </button>
      </div>
    ...
    <div>
      {gaps.map((g, i) => {
        ...
        return (
          <div
            key={i}
            className="flex items-center gap-2 px-2.5 py-[7px] border-[0.5px] border-[var(--ck-b1)] rounded-md mb-1.5 text-[12px] bg-[var(--ck-bg1)]"
          >
            <span className="w-[110px] shrink-0">{g.kp}</span>
            <div className="flex-1 h-[6px] bg-[var(--ck-bg3)] rounded-sm overflow-hidden">
              <div
            ...
            <button
              ...
              className={`w-4 h-4 rounded-[3px] border-[1.5px] flex items-center justify-center cursor-pointer text-[10px] shrink-0 transition-colors ${
                isEm
                  ? 'bg-[var(--ck-warn-bg)] border-[var(--ck-warn-t)] text-[var(--ck-warn-t)]'
                  : 'border-[var(--ck-b1)] text-transparent'
              }`}
            >
              {isEm ? '!' : ''}
            </button>
      ...
      <div className="text-[11px] text-[var(--ck-t3)] mt-1.5 leading-relaxed">
        点击 ! 标记重点关注的知识点
      </div>
      <div className="flex gap-2 justify-end mt-3.5">
        <button
...
function SummaryPanel({
  ...
  return (
    <div>
      <div className="bg-[var(--ck-bg2)] rounded-[var(--ck-rl)] px-4 py-3.5 mb-3">
        {rows.map((r, i) => (
          <div key={i} className="flex justify-between py-1 text-[13px]">
            <span className="text-[var(--ck-t2)]">{r.label}</span>
            <span className="font-semibold">{r.value}</span>
          </div>
        ))}
        {emphasisGaps.length > 0 && (
          <div className="flex justify-between py-1 text-[13px]">
            <span className="text-[var(--ck-t2)]">重点关注</span>
            <span className="font-semibold">
              {emphasisGaps.map((g, i) => (
                <span
                  key={i}
                  className="inline-block text-[11px] px-2.5 py-[3px] rounded-[10px] mx-0.5 bg-[var(--ck-warn-bg)] text-[var(--ck-warn-t)] font-medium"
                >
                  {g.kp} {g.rate}%
                </span>
              ))}
