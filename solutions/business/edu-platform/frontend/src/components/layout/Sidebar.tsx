...
import type { NavRoute } from './nav-config'
...
const navItems = NAV_ROUTES.filter((r) => r.section === 'nav')
const manageItems = NAV_ROUTES.filter((r) => r.section === 'manage')
...
export function Sidebar({ pendingCount }: SidebarProps) {
  ...
  const renderItem = (item: NavRoute) => {
    const active = isActive(item.path)
    const showBadge = item.hasBadge && pendingCount && pendingCount > 0
    return (
      <a
        key={item.label}
