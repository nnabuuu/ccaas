...
每新增一种练习类型，需手动修改以下 **6 个后端 + 7 个前端** 扩展点：
...
export interface ExercisePluginProps {
  ...
  // Per-type optional props (exercise component picks what it needs)
  [key: string]: any;
}
...
export interface ExerciseUIPlugin {
  ...
  canSubmit(exercise: Record<string, any>, ans: Record<string, any>): boolean;

  /** Format ans into the submit payload for the backend */
  formatSubmitData(ans: Record<string, any>): Record<string, any>;
...
const canSub = () => plugin.canSubmit(ex, ans);
...
### Phase 1: 迁移 quiz + match（最简单）

**原因**：quiz 和 match 没有 DI 依赖（grader 是纯函数），是最简单的迁移起点。

**步骤**：
1. 创建 `QuizPlugin` 和 `MatchPlugin`（`@Injectable() @ExerciseType('quiz')`）
...
3. 创建前端 `quizPlugin` 和 `matchPlugin`
...
**每个类型的步骤**：
1. 提取 schema → plugin 文件
2. 提取 sanitize 逻辑 → `plugin.sanitize()`
3. 迁移 grader → `plugin.grade()`
4. 迁移 buildCheckItems case → `plugin.buildCheckItems()`
5. 迁移前端 canSubmit / handleCheckResult / Component
6. 迁移前端 observe ClassView + StudentView → plugin 的 `ObserveClassView` / `ObserveStudentView`
7. 跑相关 E2E

### Phase 3: 迁移 DI 依赖类型

这些类型的 grader 需要 `AiPromptBuilder`（或其他服务）：
- `matrix`
- `map`
- `image-upload`
- `rich-content-quiz`（复用 image-upload grader）
- `guided-discovery`

**好处**：Plugin 是 `@Injectable()`，NestJS 自动注入依赖，比当前 `new XxxGrader(aiPromptBuilder)` 更干净。
