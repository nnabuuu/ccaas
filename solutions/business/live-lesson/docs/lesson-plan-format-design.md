...
Agent session 启动时, **live-lesson 通过现有的 artifact-sync 通道把项目所选 subject 的 L1 库 + 当前 user 在这些 subject 内的 L2 解读返回给 ccaas, ccaas 把它们写进 workspace 的 `artifacts/_lib/` 目录** (一个 subject 一个文件):

- `artifacts/_lib/teaching-requirements/<subject>.md` —— 该 subject 的完整 L1 库, 按 category 分 section, 每个 item 一段
- `artifacts/_lib/my-interpretations/<subject>.md` —— 当前 user 在该 subject 内的所有 L2 解读, 一个 req 一段

跨学科项目自然展开为多个文件 (每个 subject 一对); 单学科项目就是一对。Subject 列表由 `course_projects.subjects[]` 决定 —— 创建项目时挑选, 后期可改。空数组 = 不物化任何库 (项目不依赖教学要求库)。

Agent 用标准工具访问:

```
Read   artifacts/_lib/teaching-requirements/english.md      # 翻指定学科
Grep -r "推断生词" artifacts/_lib/teaching-requirements/    # 跨所有 subject 找 id
Grep -r "r-1.2.3" artifacts/_lib/my-interpretations/        # 跨学科查自己的解读
```
...
- L2 是 *per-user* 数据, 不进 lesson plan, 只能通过 materialized `_lib/my-interpretations/<subject>.md` 拿 (按项目所选 subject 过滤; 跨学科解读不会泄露给非该 subject 的项目)

#### Materialization 时机

- **Session 启动时**: ccaas 调 live-lesson `GET /api/projects/:id/artifacts` (带 `X-Caller-User-Id` header); live-lesson 端 `listArtifactsWithContent` 遍历 `project.subjects[]`, 为每个 subject 渲染一对 `_lib/teaching-requirements/<subject>.md` + `_lib/my-interpretations/<subject>.md` 并放进响应
...
没 bash 的 agent 用 §4.1 路径 B 的 `Grep -r artifacts/_lib/` 拿到等价信息, 只是要多一步 (find id → look up interpretation)。
...
`listArtifactsWithContent` 在 artifact-sync 时把当前 user 在项目所选 subject 内的 L2 解读写入 `_lib/my-interpretations/<subject>.md` (一个 subject 一个文件):

```markdown
<!--
你的解读 (per-user overlay, 来自 user X). 编辑请回到 Plan Tab popover.
查找用 Grep, 例如: Grep -r "r-1.2.3" artifacts/_lib/my-interpretations/
-->
...
Agent 用 `Read artifacts/_lib/my-interpretations/<subject>.md` 或 `Grep -r "<id>" artifacts/_lib/my-interpretations/` 即可, **不需要 bash**。
...
**ccaas 侧改动 (跨项目):**
- `packages/agent-runtime/src/artifact/workspace-artifact-source.ts` —— `loadArtifacts(projectId, context?: { userId? })` 增加可选 context, 让 solution 端 user-scope 返回内容
- `packages/backend/src/sessions/agent-runtime/{rest-workspace-artifact-source.ts,session-asset-syncer.service.ts}` —— 把 session 的 userId 通过 `X-Caller-User-Id` header 透传给 solution 的 artifact endpoint
- ccaas-core **没有** lesson-plan / teaching-requirement / subject 等领域名词; lib materialization 完全发生在 live-lesson 的 `listArtifactsWithContent` 内, 一个 subject 一对 `_lib/teaching-requirements/<subject>.md` + `_lib/my-interpretations/<subject>.md`
...
4. ~~**Subject 怎么定**? 用 `manifest.json` 已有的 `subject` 字段~~ **已落地**: `course_projects.subjects: string[]` 列 (项目级, 创建时挑选, 可后续编辑, 支持跨学科)。Lesson manifest 上的 `subject` 字段仍存在但只用于 lesson 发布元数据, 不参与库 materialization
