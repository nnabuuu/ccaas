...
function ObservationPanel({ state }: { state: ClassroomState }) {
  const [mode, setMode] = useState<ObsMode>('glance')
  const obs = state.observation

  if (!obs || (obs.logs.length === 0 && obs.alerts.length === 0 && obs.anchorStats.length === 0)) {
    return (
      <div>
  ...
  return (
    <div>
      ...
      {mode === 'glance' && <GlanceView obs={obs} students={state.students} />}
...
function deriveObsColor(obs: ClassroomState['observation'], studentId: string): string {
  ...
  const now = Date.now()
  if (now - log.systemMetrics.lastActiveAt > 3 * 60 * 1000) return 'var(--t3)' // idle
...
function GlanceView({ obs, students }: {
  obs: NonNullable<ClassroomState['observation']>
  students: ClassroomState['students']
}) {
  const knowledgeAnchors = obs.anchorStats.filter(a => a.type === 'knowledge')
  const misconceptionAnchors = obs.anchorStats.filter(a => a.type === 'misconception').sort((a, b) => b.studentCount - a.studentCount)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      {/* Knowledge anchor progress */}
      {knowledgeAnchors.length > 0 && (
        <div className="patterns" style={{ gridTemplateColumns: '1fr' }}>
          <div className="pat">
            <div className="pat-h info"><span className="dot" />知识锚点</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 4 }}>
              {knowledgeAnchors.map(a => (
                <div key={a.anchorId} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ fontSize: 9, fontWeight: 600, color: 'var(--t2)', width: 22, flexShrink: 0 }}>{a.anchorId}</span>
      ...
      {/* Misconception anchors sorted by impact */}
      {misconceptionAnchors.some(a => a.studentCount > 0) && (
        <div className="patterns" style={{ gridTemplateColumns: '1fr' }}>
          <div className="pat alert">
            <div className="pat-h warn"><span className="dot" />误解信号</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 4 }}>
              {misconceptionAnchors.filter(a => a.studentCount > 0).map(a => (
                <div key={a.anchorId}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontSize: 9, fontWeight: 600, color: 'var(--amber)', width: 22, flexShrink: 0 }}>{a.anchorId}</span>
