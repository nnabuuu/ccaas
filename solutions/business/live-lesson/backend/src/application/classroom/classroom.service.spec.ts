...
import { DiscussService } from '../ai/discuss.service';
import { AiAskService } from '../ai/ai-ask.service';
...
describe('ClassroomService — persistence', () => {
  ...
  describe('with a session and student', () => {
    ...
    it('should persist AI questions in DB', async () => {
      ...
      // Direct DB insert bypasses service-level markDirty, so invalidate cache manually
      module.get(StateCacheService).markDirty(session.id);
      const state = await service.getState(session.id);
