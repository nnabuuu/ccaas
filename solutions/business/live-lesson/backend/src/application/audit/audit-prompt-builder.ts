...
:::warn[标题]
需关注的描述, 自然语言。可以引用模块 [Step 1](nav://execution/s-1700000001-3) 或要求 [r-1.2.3](nav://plan/r-1.2.3)。
内嵌动作按钮: [让 AI 修复](action://fix?target=s-1700000001-3&issue=missing-coverage) [跳转查看](nav://execution/s-1700000001-3)
:::
...
- \`nav://execution/<stepId>\` —— stepId 是 manifest 里 step 的 \`id\` 字段 (e.g. \`s-1700000001-3\`), 直接当 anchor。 教师在链接<em>文本</em>里看到的还是 "Step 1" / "Step 2" 之类的, URL 部分是 stepId。 这样 reorder / edit step 后链接仍指向原 step; 用户删除了那个 step 时链接静默失效 (不会错跳到别的 step)。
