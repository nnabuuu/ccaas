...
:::warn[标题]
需关注的描述, 自然语言。可以引用模块 [Step 1](nav://execution/step-1|s-1700000001-3) 或要求 [r-1.2.3](nav://plan/r-1.2.3)。
内嵌动作按钮: [让 AI 修复](action://fix?target=step-1&issue=missing-coverage) [跳转查看](nav://execution/step-1|s-1700000001-3)
:::
...
nav:// 链接 anchor 格式 (前端会根据 anchor 自动滚动到目标):
- \`nav://execution/step-N|<stepId>\` —— **推荐双 anchor 形式**。N 是 step 的 1-based 编号 (readingSteps 数组下标 + 1; 教师视觉编号), stepId 是 manifest 里该 step 的 \`id\` 字段 (e.g. \`s-1700000001-3\`)。 用竖线 \`|\` 分隔。 前端优先用 stepId 匹配 (reorder 后仍能定位), stepId 找不到时兜底用 N。
- \`nav://execution/step-N\` —— 简略形式, 没有 stepId。 reorder 后会跳到位置上的 step (不是审计当时的)。 不推荐, 但向后兼容。
- \`nav://plan/<reqId>\` —— reqId 是教学要求的完整 id (不带 \`req://\` 前缀), e.g. \`nav://plan/r-1.2.3\`。
- \`nav://skills\` —— 仅切到 Skills 工作区, 没有 anchor。
- \`nav://execution\` 或 \`nav://plan\` (不带 anchor) —— 仅切 tab, 不滚动。

输出约束:
- 只返回 markdown 文档, 不要 JSON 包装、不要解释、不要前言后语。
- 文档以 \`# 概述\` 开头。
- 报告语言简体中文, 教师本人能读懂。
- action://, req:// 链接的具体路径让 LLM 自己根据上下文填合理值, 前端会忽略点不通的链接。`;
