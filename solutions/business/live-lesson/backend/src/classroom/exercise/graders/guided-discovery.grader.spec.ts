...
describe('GuidedDiscoveryGrader — image blanks', () => {
  ...
  const imageKey: GuidedDiscoveryAnswerKey = {
    ...
    steps: [{
      ...
      title: '写出公式',
      blanks: [
        { id: 'lhs', label: '左边', accepts: ['(a+b)(a-b)'], rejects: ['a+b'] },
        { id: 'rhs', label: '右边', accepts: ['a²-b²', 'a^2-b^2'] },
      ],
    }],
  };

  it('calls vision LLM for data URI answer and marks correct', async () => {
    const ai = mockAiPromptBuilder(JSON.stringify({
      recognized: '(a+b)(a-b)',
      correct: true,
      feedback: '正确！',
    }));
  ...
  it('marks incorrect when LLM says correct=false', async () => {
    const ai = mockAiPromptBuilder(JSON.stringify({
      recognized: 'a+b',
      correct: false,
      feedback: '需要写完整的乘法',
  ...
  it('includes rejects in LLM prompt for formula_blanks', async () => {
    const ai = mockAiPromptBuilder(JSON.stringify({
      recognized: '(a+b)(a-b)',
      correct: true,
      feedback: '正确',
    ...
    expect(userContent.text).toContain('常见错误：a+b');
  });

  it('derivation_blank image answers are graded via LLM', async () => {
    const ai = mockAiPromptBuilder(JSON.stringify({
      recognized: 'a²-b²',
      correct: true,
      feedback: '正确',
  ...
  it('text_blanks image answers are graded via LLM', async () => {
    const ai = mockAiPromptBuilder(JSON.stringify({
      recognized: '和',
      correct: true,
      feedback: '正确',
  ...
  it('handles code-fence wrapped JSON response', async () => {
    const ai = mockAiPromptBuilder('```json\n{"recognized":"(a+b)(a-b)","correct":true,"feedback":"正确"}\n```');
    const g = new GuidedDiscoveryGrader(ai);
    const data = {
      steps: { sym: { answers: { lhs: FAKE_IMAGE, rhs: 'a²-b²' } } },
    };
  ...
  it('both blanks as images triggers two LLM calls', async () => {
    const ai = {
      callVisionLlm: jest.fn()
        .mockResolvedValueOnce(JSON.stringify({ recognized: '(a+b)(a-b)', correct: true, feedback: '左边正确' }))
        .mockResolvedValueOnce(JSON.stringify({ recognized: 'a²-b²', correct: true, feedback: '右边正确' })),
    } as unknown as AiPromptBuilder;
    const g = new GuidedDiscoveryGrader(ai);
    const data = {
    ...
    // Feedback from all image blanks is collected (even correct ones)
    expect(result.llmFeedback).toBe('左边正确; 右边正确');
  });

  it('both blanks as images, both wrong — feedbacks joined with semicolon', async () => {
    const ai = {
      callVisionLlm: jest.fn()
        .mockResolvedValueOnce(JSON.stringify({ recognized: 'x', correct: false, feedback: '左边错误' }))
        .mockResolvedValueOnce(JSON.stringify({ recognized: 'y', correct: false, feedback: '右边错误' })),
    } as unknown as AiPromptBuilder;
    const g = new GuidedDiscoveryGrader(ai);
    const data = {
      steps: { sym: { answers: { lhs: FAKE_IMAGE, rhs: FAKE_IMAGE } } },
    ...
    const result = await g.grade(imageKey, data);
    expect(result.byDimension.sym).toBe(false);
    expect(result.llmFeedback).toBe('左边错误; 右边错误');
  });
  ...
  it('multiple steps with images — feedbacks from different steps joined with newline', async () => {
    const ai = {
      callVisionLlm: jest.fn()
        .mockResolvedValueOnce(JSON.stringify({ recognized: 'x', correct: false, feedback: '公式步骤错误' }))
        .mockResolvedValueOnce(JSON.stringify({ recognized: 'y', correct: false, feedback: '验证步骤错误' })),
    } as unknown as AiPromptBuilder;
    const g = new GuidedDiscoveryGrader(ai);
    ...
    expect(result.llmFeedback).toBe('公式步骤错误\n验证步骤错误');
  });

  it('LLM returns correct as string "true" — rejected by strict equality', async () => {
    const ai = mockAiPromptBuilder(JSON.stringify({
      recognized: '(a+b)(a-b)',
      correct: 'true',
      feedback: '正确',
    ...
    const result = await g.grade(imageKey, data);
    // "true" !== true, so blank is marked incorrect
    expect(result.byDimension.sym).toBe(false);
  });

  it('LLM returns JSON missing correct field — treated as incorrect', async () => {
    const ai = mockAiPromptBuilder(JSON.stringify({
      recognized: '(a+b)(a-b)',
      feedback: '看不清楚',
    }));
  ...
  it('LLM returns correct without feedback — no feedback appended', async () => {
    const ai = mockAiPromptBuilder(JSON.stringify({
      recognized: '(a+b)(a-b)',
      correct: true,
    }));
  ...
  it('prompt omits rejects line when blank has no rejects', async () => {
    const ai = mockAiPromptBuilder(JSON.stringify({
      recognized: 'a²-b²',
      correct: true,
      feedback: '正确',
    ...
    const g = new GuidedDiscoveryGrader(ai);
    // rhs blank has no rejects
    const data = {
      steps: { sym: { answers: { lhs: 'a²-b²', rhs: FAKE_IMAGE } } },
    };
    await g.grade(imageKey, data);
    const callArgs = (ai.callVisionLlm as jest.Mock).mock.calls[0];
    const userContent = callArgs[1].find((c: { type: string }) => c.type === 'text');
  ...
  it('buildCheckItems includes llmFeedback for image grading', async () => {
    ...
    expect(llmItem!.hint).toContain('需要完整乘法');
  });
});
