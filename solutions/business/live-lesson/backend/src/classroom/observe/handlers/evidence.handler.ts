...
export class EvidenceObserveHandler implements ObserveHandler {
  compute(ctx: ObserveContext): EvidenceObserveData {
    if (ctx.answerKey && ctx.answerKey.type !== 'select-evidence') {
      throw new Error(`EvidenceObserveHandler expects select-evidence answerKey, got ${ctx.answerKey.type}`);
    }
