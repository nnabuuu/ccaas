/* ═══════════════════════════════════════════════════════════
   Teacher Console v5 — Root App
   Orchestrates views: main → discuss class → discuss student
   ═══════════════════════════════════════════════════════════ */

function TeacherApp() {
  const [view, setView] = useState('main'); // 'main' | 'discuss-class' | 'discuss-student'
  ...
  return React.createElement('div', { style:{ display:'flex', flexDirection:'column', height:'100vh' } },
    ...
    /* ═══ BODY ═══ */
    view === 'main' ? React.createElement('div', { style:{ flex:1, display:'grid', gridTemplateColumns:'1fr 340px', minHeight:0 } },
      /* Left */
      React.createElement('div', { style:{ padding:'18px 24px', overflowY:'auto', display:'flex', flexDirection:'column', gap:14 } },
        React.createElement(HealthCards),
        React.createElement(StepsSection, { selectedSignal, onCompClick:handleCompClick }),
      ),
      /* Right */
      React.createElement(RightPanel, { selectedSignal, onSelectSignal:setSelectedSignal }),
    )
    : view === 'discuss-class' ? React.createElement('div', { style:{ flex:1, display:'grid', gridTemplateColumns:'1fr 340px', minHeight:0 } },
