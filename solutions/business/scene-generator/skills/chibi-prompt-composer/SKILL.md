---
name: "Chibi Prompt Composer"
slug: "chibi-prompt-composer"
description: "分析文章文本，组装4层chibi场景prompt（不生成图片）"
type: prompt
version: "1.0.0"
triggers:
  - type: keyword
...
**重要：你只负责生成 prompt 文本，不要调用任何图片生成工具。**
...
**不要调用 generate_image 工具。** 将组装好的完整 prompt 以格式化文本输出给用户：
...
然后提示用户：
> Prompt 已就绪！你可以审查和修改上面的 prompt。确认后请说「生成图片」来生成插图。

# 关键原则

1. **不调用工具** — 本 skill 只输出 prompt 文本，不调用 generate_image。
2. 角色锁定不可妥协。每个prompt都以完整角色枚举开头。
3. 默认半身。只在构图特别需要且有取景方案时才全身。
4. 表情要克制。Nano Banana默认会夸张化。
5. 多面板 = 竖版。永远不要对chibi角色用左右分割。
6. 文字后期加。除了经典meme文字，所有标注都在Figma/Canva加。
