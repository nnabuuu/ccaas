...
export default function DimensionControls() {
  ...
  return (
    <div>
      <label className="text-xs font-medium text-gray-600 block mb-1.5">Dimensions</label>
      ...
      <div className="space-y-1.5">
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-gray-500 w-5">W</span>
          <input
            type="range"
            min={8}
            max={128}
          ...
          />
          <span className="text-xs text-gray-700 w-8 text-right">{config.widthStuds}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-gray-500 w-5">H</span>
          <input
            type="range"
            min={8}
            max={128}
            ...
            className="flex-1 h-1 accent-blue-500"
          />
          <span className="text-xs text-gray-700 w-8 text-right">{config.heightStuds}</span>
        </div>
      </div>
      ...
      <button
        ...
        className={`mt-1 text-[10px] px-2 py-0.5 rounded transition-colors ${
          locked ? 'bg-blue-50 text-blue-600' : 'bg-gray-100 text-gray-500'
        }`}
      >
